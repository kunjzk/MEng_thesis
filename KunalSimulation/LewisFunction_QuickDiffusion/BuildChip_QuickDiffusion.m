...
function [chip_layout, sensor_location, sensor_plotlocation] = BuildChip_QuickDiffusion(x, y, chip, resolution)
    ...
    %Row of sensor_location denotes each sensor, columns denote coordinates
    %that bound sensing area: x_start, x_end, y_start, y_end
    sensor_location = zeros(chip.N_x*chip.N_y, 4); 
    ...
    for i = 1:chip.N_x
        for j = 1:chip.N_y
            ...
            %Save coordinates
            sensor_location((i-1)*chip.N_y + j, 1) = sensor_xstart;
            sensor_location((i-1)*chip.N_y + j, 2) = sensor_xend;
            sensor_location((i-1)*chip.N_y + j, 3) = sensor_ystart;
            sensor_location((i-1)*chip.N_y + j, 4) = sensor_yend;
