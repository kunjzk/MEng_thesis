...
figure
plot(t1, sum(cum_nH_1), 'DisplayName', 'Frame rate = 1')
hold on
plot(t3, sum(cum_nH_3), 'DisplayName', 'Frame rate = 3')
plot(t5, sum(cum_nH_5), 'DisplayName', 'Frame rate = 5')
plot(t10, sum(cum_nH_10), 'DisplayName', 'Frame rate = 10')
hold off
grid on 
grid minor
legend
...
%     plot(t, del_v)
%     hold on
%     plot(t, mean(del_v), 'LineWidth', 3, 'Color', 'Black')
%     hold off
...
%     
%     plotSpatialVoltageFrame(final_neg, chip, frame_rates(counter), ceil(510/frame_rates(counter)))
%     plotSpatialVoltageFrame(final_neg, chip, frame_rates(counter), ceil(511/frame_rates(counter)))
    
    if counter == 1
        diffv1 = [diff(mean(final_neg))];
    end

    if counter == 2
        diffv3 = [diff(mean(final_neg))];
    end

    if counter == 3
        diffv5 = [diff(mean(final_neg))];
    end
    
    if counter == 4
        diffv10 = [diff(mean(final_neg))];
    end


    diffv = [diff(mean(final_neg))];
    figure
...
%%
figure
plot(1:1:length(diffv1), diffv1.*-1, 'LineWidth', 2, 'DisplayName', 'F=1');
hold on
plot(1:1:length(diffv3), diffv3.*-1, 'LineWidth', 2, 'DisplayName', 'F=3');
plot(1:1:length(diffv5), diffv5.*-1, 'LineWidth', 2, 'DisplayName', 'F=5');
plot(1:1:length(diffv10), diffv10.*-1, 'LineWidth', 2, 'DisplayName', 'F=10');
hold off
title('Mean Absolute Difference in Noise Across Frames')
legend
grid on
grid minor
a = get(gca,'XTickLabel');  
