...
function [chip_layout, column_location, row_location, sensor_plotlocation] = BuildChip_QuickDiffusionv3(x, y, chip, resolution)
    ...
    electrode_ystart = chip.wall_separation_yneg;
    electrode_yend = chip.wall_separation_yneg + chip.N_x*chip.isfet_length + (chip.N_x-1)*chip.isfetisfet_separation + chip.sensor_startdisplacement + chip.sensor_enddisplacement;
    electrode_yindex_linear = find(y >= electrode_ystart & y <= electrode_yend);
    ...
    for i = 1:chip.N_electrodes
        ...
        electrode_xstart = chip.wall_separation_xneg + (i-1)*(chip.electrode_width + 2*chip.electrodeisfet_separation + chip.isfet_width + 2*chip.castellation_extension);
        electrode_xend = chip.wall_separation_xneg + i*chip.electrode_width + (i-1)*(2*chip.electrodeisfet_separation + chip.isfet_width + 2*chip.castellation_extension);
        ...
        %Convert x, y co-ordinates to linear indices for easy storage
        for k = 1:length(electrode_xindex_linear)
            for l = 1:length(electrode_yindex_linear)

                %Add to chip and sensor layout
                chip_layout(electrode_xindex_linear(k), electrode_yindex_linear(l), 1) = 0;
                chip_layout(electrode_xindex_linear(k), electrode_yindex_linear(l), 2) = 1;
                chip_layout(electrode_xindex_linear(k), electrode_yindex_linear(l), 3) = 0;
            end
        end
    end  
end
